...
$select = "SELECT cs.eCodSubclasificacion,cs.tNombre FROM CatSubClasificacionesInventarios WHERE eCodTipoInventario=$eCodTipoInventario";
$rsSubclasificaciones = mysql_query($select);

while($rSubclasificacion = mysql_fetch_array($rsSubclasificaciones))
{
   $valores .= '<option value="'.$rSubclasificacion{'eCodSubclasificacion'}.'">'.utf8_endoce($rSubclasificacion{'tNombre'}).'</option>'; 
}
